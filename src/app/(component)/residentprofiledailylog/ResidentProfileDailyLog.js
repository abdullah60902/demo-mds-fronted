...
const ResidentProfileDailyLog = ({ clientId }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://demo-mds-backend.vercel.app/daily-log/client/${clientId}`, {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("token");
    fetch(`https://demo-mds-backend.vercel.app/daily-log/older-than-six-months`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`https://demo-mds-backend.vercel.app/daily-log/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
