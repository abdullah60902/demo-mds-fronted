...
const ResidentProfileRiskAssessment = ({ clientId, userRole }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://demo-mds-backend.vercel.app/risk-assessment/client/${clientId}`, {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("token");
    fetch(`https://demo-mds-backend.vercel.app/risk-assessment/older-than-six-months`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`https://demo-mds-backend.vercel.app/risk-assessment/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
