...
const tabs = [
  ...
  const toggleExportModule = (id) => {
    ...
    try {
        ...
        const freshClientRes = await fetch(`http://localhost:3000/client/${id}`, config);
        const freshClient = await freshClientRes.json();
        // Also re-fetch care plans
        let freshCarePlans = clientcareplane;
        try {
            const cpRes = await fetch(`http://localhost:3000/carePlanning/client/${id}`, config);
        ...
        if (selectedExportModules.includes("documents")) {
            ...
            yPos += 8;
            const res = await fetch(`http://localhost:3000/resident-documents/client/${id}`, config);
        ...
        if (selectedExportModules.includes("assessment")) {
            const res = await fetch(`http://localhost:3000/assessment/client/${id}`, config);
