...
const handleSubmit4 = (e) => {
  ...
  const request = editingUserId
    ? axios.put(`https://demo-mds-backend.vercel.app/social/${editingUserId}`, data, config)
    : axios.post(`https://demo-mds-backend.vercel.app/social`, data, config);
  ...
  request
    .then(res => {
      ...
      // ✅ FETCH updated data
      return axios.get('https://demo-mds-backend.vercel.app/social', config);
    })
    .then(res => {
  ...
  useEffect(() => {
    axios.get('https://demo-mds-backend.vercel.app/social', {
      headers: {
...
const handleCancel12 = () => {
  ...
  const handleDelete = (id) => {
    ...
    const token = localStorage.getItem('token');
    axios.delete(`https://demo-mds-backend.vercel.app/social/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      }
    })
...
useEffect(() => {
  const token = localStorage.getItem('token');
  axios.get('https://demo-mds-backend.vercel.app/client', {
    headers: {
      Authorization: `Bearer ${token}`,
    }
  })
